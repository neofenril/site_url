...
function traiteURL(){
	...
	$courte = "http://".$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']."?rac=";
...
function getURL(){
	...
	if (isset($_GET['rac'])) {
		$courte = trim($_GET['rac']);
		if(Url::courte_existe($p, $courte)){
			$redirection = Url::retour_url($p, $courte);
